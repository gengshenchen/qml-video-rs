...
fn main() {
    ...
    if let Ok(path) = download_and_extract(&entry.0, &format!("{}/{}", entry.1, entry.2)) {
        ...
        if target_os == "android" {
            ...
            // std::fs::copy(format!("{}/lib/arm64-v8a/libmdk.so.dsym", path), format!("{}/../../../libmdk.so.dsym", env::var("OUT_DIR").unwrap())).unwrap();
...
fn download_and_extract(url: &str, check: &str) -> Result<String, std::io::Error> {
    ...
    // 2. 配置 ConfigBuilder
    // ureq 3.x 中，先构建 Config，再用 Config 创建 Agent
    let mut config_builder = ureq::Agent::config_builder();

    if let Some(proxy_str) = proxy_url {
        println!("cargo:warning=Proxy detected, applying: {}", proxy_str);
        if let Ok(proxy) = ureq::Proxy::new(&proxy_str) {
            config_builder = config_builder.proxy(Some(proxy));
        } else {
            println!("cargo:warning=Invalid proxy URL format, attempting direct connection.");
        }
    }

    // 【修复点2】正确创建 Agent
    // config_builder.build() 返回的是 Config，不是 Agent
